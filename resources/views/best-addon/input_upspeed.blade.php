...
@endsection
@section('right_title')
	Best Add-On / Upgrade Speed / Input 
@endsection
@section('content')
		...
		<div class="row">
			<div class="col-sm-11">
				...
				<div class="form-inline">
	                    <form class="search-form" method="post" nama="blnn" action="{{url('/searchinputupspeed')}}">
	                        {{csrf_field()}}
						<select name="bln">
	            					...
	            					<tr>
	            						...
	            						@for($x=1;$x<=$jmlhari;$x++)
	            							<?php
	            								...
	            								$jml=App\addon::select(DB::raw('SUM(input_upspeed) as total_inpupspeed'))->where('witel',$wit->witel)->groupby('witel')->where('tanggal',$thn."-".$bln."-".$tgl)->first();
	            							?>
	            							@if($jml == null)
	            								<td>0</td>
	            							@else
												<td>{{$jml->total_inpupspeed}}</td>
											@endif
			            				@endfor

			            				@if($wit->total_inpupspeed==null)
                        						<td align="center" valign="middle">0</td>
                        				@else
                        						<td align="center" valign="middle">{{$wit->total_inpupspeed}}</td>
                        				@endif
	            					</tr>
	            				@endforeach      
	            			</tbody>
				                ...
				                <div class="row">
				                    ...
				                    <div class="col-xs-6 col-sm-6">
				                        <div class="card">
				                            ...
				                            <form method="post" action="{{url('/input_upspeed')}}">
				                                {{csrf_field()}}
				                            	<div class="card-body card-block">
					                                ...
					                                <div class="form-group">
					                                    <label class=" form-control-label">Jumlah Input Upgrade Speed</label>
					                                    <div class="input-group">
					                                        <div class="input-group-addon"><i class="fa fa-usd"></i></div>
					                                        <input type="text" name="input_upspeed" class="form-control">
					                                    </div>
