...
@endsection
@section('right_title')
	Best Add-On / Telepon Mania / Input 
@endsection
@section('content')
		...
		<div class="row">
			<div class="col-sm-11">
				...
				<div class="form-inline">
	                    <form class="search-form" method="post" nama="blnn" action="{{url('/searchinputtelepon')}}">
	                        {{csrf_field()}}
						<select name="bln">
	            					...
	            					<tr>
	            						...
	            						@for($x=1;$x<=$jmlhari;$x++)
	            							<?php
	            								...
	            								$jml=App\addon::select(DB::raw('SUM(input_telepon) as total_inptelepon'))->where('witel',$wit->witel)->groupby('witel')->where('tanggal',$thn."-".$bln."-".$tgl)->first();
	            							?>
	            							@if($jml == null)
	            								<td>0</td>
	            							@else
												<td>{{$jml->total_inptelepon}}</td>
											@endif
			            				@endfor

			            				@if($wit->total_inptelepon==null)
                        						<td align="center" valign="middle">0</td>
                        				@else
                        						<td align="center" valign="middle">{{$wit->total_inptelepon}}</td>
                        				@endif
	            					</tr>
	            				@endforeach      
	            			</tbody>
				                ...
				                <div class="row">
				                    ...
				                    <div class="col-xs-6 col-sm-6">
				                        <div class="card">
				                            ...
				                            <form method="post" action="{{url('/input_telepon')}}">
				                                {{csrf_field()}}
				                            	<div class="card-body card-block">
					                                ...
					                                <div class="form-group">
					                                    <label class=" form-control-label">Jumlah Input Telepon Mania</label>
					                                    <div class="input-group">
