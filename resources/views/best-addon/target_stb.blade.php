...
@endsection
@section('right_title')
	Best Add-On / STB Tambahan / Target
@endsection
@section('content')
		...
		<div class="row">
			<div class="col-sm-11">
				...
				<div class="form-inline">
	                    <form class="search-form" method="post" nama="blnn" action="{{url('/searchtargetstb')}}">
	                        {{csrf_field()}}
						<select name="bln">
	            					...
	            					<tr>
	            						...
	            						@for($x=1;$x<=$jmlhari;$x++)
	            							<?php
	            								...
	            								$jml=App\addon::select(DB::raw('SUM(target_stb) as total_targetstb'))->where('witel',$wit->witel)->groupby('witel')->where('tanggal',$thn."-".$bln."-".$tgl)->first();
	            							?>
	            							@if($jml == null)
	            								<td>0</td>
	            							@else
												<td>{{$jml->total_targetstb}}</td>
											@endif
			            				@endfor

			            				@if($wit->total_targetstb==null)
                        						<td align="center" valign="middle">0</td>
                        				@else
                        						<td align="center" valign="middle">{{$wit->total_targetstb}}</td>
                        				@endif
	            					</tr>
	            				@endforeach      
	            			</tbody>
				                ...
				                <div class="row">
				                    ...
				                    <div class="col-xs-6 col-sm-6">
				                        <div class="card">
				                            ...
				                            <form method="post" action="{{url('/target_stb')}}">
				                                {{csrf_field()}}
				                            	<div class="card-body card-block">
					                                ...
					                                <div class="form-group">
					                                    <label class=" form-control-label">Jumlah Target STB Tambahan</label>
					                                    <div class="input-group">
					                                        <div class="input-group-addon"><i class="fa fa-usd"></i></div>
					                                        <input type="text" name="target_stb" class="form-control">
					                                    </div>
