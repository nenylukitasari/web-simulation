		...
		<div class="row">
			<div class="col-sm-11">
				...
				<div class="form-inline">
	                    <form class="search-form" method="post" nama="blnn" action="{{url('/searchsalespull')}}">
	                        {{csrf_field()}}
						<select name="bln">
			...
			<div class="col-md-12">
	            <div class="card">
	                <div class="card-header">
	                   <strong class="card-title">Data Sales Pull bulan ke - {{$bln}}</strong>
	                </div>
	                <div class="card-body">
	            					...
	            					<tr>
	            						...
	            						@for($x=1;$x<=$jmlhari;$x++)
	            							<?php
	            								...
	            								$jml=App\pullput::select(DB::raw('SUM(salespull) as total_salespull'))->where('witel',$wit->witel)->groupby('witel')->where('tanggal',$thn."-".$bln."-".$tgl)->first();
	            							?>
	            							@if($jml == null)
	            								<td>0</td>
	            							@else
												<td>{{$jml->total_salespull}}</td>
											@endif
			            				@endfor

			            				@if($wit->total_salespull==null)
                        						<td align="center" valign="middle">0</td>
                        				@else
                        						<td align="center" valign="middle">{{$wit->total_salespull}}</td>
                        				@endif
	            					</tr>
	            				@endforeach      
	            			</tbody>
				                ...
				                <div class="row">
				                    ...
				                    <div class="col-xs-6 col-sm-6">
				                        <div class="card">
				                            ...
				                            <form method="post" action="{{url('/salespull')}}">
				                                {{csrf_field()}}
				                            	<div class="card-body card-block">
					                                ...
					                                <div class="form-group">
					                                    <label class=" form-control-label">Jumlah Sales Pull</label>
					                                    <div class="input-group">
					                                        <div class="input-group-addon"><i class="fa fa-usd"></i></div>
					                                        <input type="text" name="salespull" class="form-control">
					                                    </div>
