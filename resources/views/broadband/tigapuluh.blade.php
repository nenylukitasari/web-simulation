		...
		<div class="row">
			<div class="col-sm-11">
				...
				<div class="form-inline">
	                    <form class="search-form" method="post" nama="blnn" action="{{url('searchtigapuluh')}}">{{-- input ndes --}}
	                        {{csrf_field()}}
						<select name="bln">
			...
			<div class="col-md-12">
	            <div class="card">
	                <div class="card-header">
	                   <strong class="card-title">Data PS IH 30M bulan ke - {{$bln}}</strong>
	                </div>
	                <div class="card-body">
	            					...
	            					<tr>
	            						...
	            						@for($x=1;$x<=$jmlhari;$x++)
	            							<?php
	            								...
	            								$jml=App\broadband::select(DB::raw('SUM(tigapuluh) as total_tigapuluh'))->where('witel',$wit->witel)->groupby('witel')->where('tanggal',$thn."-".$bln."-".$tgl)->first();
	            							?>
	            							@if($jml == null)
	            								<td>0</td>
	            							@else
												<td>{{$jml->total_tigapuluh}}</td>
											@endif
			            				@endfor

			   							@if($wit->total_tigapuluh==null)
                        						<td align="center" valign="middle">0</td>
                        					@else
                        						<td align="center" valign="middle">{{$wit->total_tigapuluh}}</td>
                        					@endif
				                ...
				                <div class="row">
				                    ...
				                    <div class="col-xs-6 col-sm-6">
				                        <div class="card">
				                            ...
				                            <form method="post" action="{{url('best-broadband/inputtigapuluh')}}">
				                                {{csrf_field()}}
				                            	<div class="card-body card-block">
					                                ...
					                                <div class="form-group">
					                                    <label class=" form-control-label">PS IH 30M</label>
					                                    <div class="input-group">
					                                        <div class="input-group-addon"><i class="fa fa-usd"></i></div>
					                                        <input type="text" name="tigapuluh" class="form-control">
					                                    </div>
